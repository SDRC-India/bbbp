...
@Component({
  selector: 'app-exposurevisits',
  templateUrl: './exposurevisits.component.html',
  styleUrls: ['./exposurevisits.component.scss']
})
export class ExposurevisitsComponent implements OnInit {
  subSection: any[] = [];
  selectedItem: any;
  isDesc:any;
  column:any;
  direction:number;
  showExposureVisitsData: Boolean;
  p: number = 1;
  url: any;
  searchTexts: any;
  f_sl:number = 1;
  ...
  constructor(private staticServiceProvider:StaticPageService, private sanitizer: DomSanitizer) {
   ...
   ngOnInit() {
    this.staticService.getData("Exposure Visits").subscribe(data=>{
  ...
  getYearData(index: any, event, newValue){
    ...
    this.subSection = this.staticService.nestedSection[index].slice().reverse();;
    if(Object.keys(this.subSection[0]).length === 0){
      this.showExposureVisitsData = false;
    }else{
      this.showExposureVisitsData = true;
    }
   }
